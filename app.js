...
function applyLangButtons() {
  ['lang-fr-btn','lang-fr-btn-2'].forEach(idn => { const el = document.getElementById(idn); if (el) el.classList.toggle('active', currentLang === 'fr'); });
  ['lang-en-btn','lang-en-btn-2'].forEach(idn => { const el = document.getElementById(idn); if (el) el.classList.toggle('active', currentLang === 'en'); });
}
...
function renderHome() {
  ...
  document.getElementById('home-progress-text').textContent = `${completed} / ${MODULES.length}`;
...
function renderModule() {
  ...
  document.getElementById('mod-eyebrow').textContent = `Module ${m.id} · ${m.duration}`;
